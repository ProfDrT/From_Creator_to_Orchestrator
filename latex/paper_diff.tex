...
%DIF DEL /tmp/paper_baseline.tex   Mon Mar  2 16:49:23 2026
%DIF ADD latex/paper.tex           Mon Mar  2 16:48:41 2026
...
%DIFDELCMD <   %%%
\DIFdelend \DIFaddbegin \And
  \DIFaddend Burkhardt Funk\\
  Leuphana University\\
  L\"uneburg, Germany\\
...
\DIFdel{The text of this paper was generated by Claude Opus 4.6 (Anthropic PBC). The human authors designed the research, selected theoretical lenses, evaluated all outputs, verified citations, and take full responsibility for the published content. Full pipeline artifacts are archived at the project repository.}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \parbox{\dimexpr\textwidth-2\fboxsep-2\fboxrule}{%DIF > 
%DIF >  [R2-02: AI Disclosure clarified]
\textbf{\DIFadd{AI Disclosure:}} \DIFadd{The text of this paper was generated by Claude Opus 4.6 (Anthropic PBC). The human authors designed the research, directed theoretical framing, evaluated all outputs, and take responsibility for the published content. Full pipeline artifacts are archived at the project repository.}}\DIFaddend }

\vspace{1em}

\DIFdelbegin %DIFDELCMD < \keywords{autonomous AI agents \and orchestration \and knowledge work transformation \and LLM-based research automation \and Constitutional AI \and distributed cognition \and sociotechnical systems \and scientific labor \and Model Context Protocol}
%DIFDELCMD < %%%
\DIFdelend %DIF >  [R3: Keywords reduced per Burkhardt — "zu kleinteilig"]
\DIFaddbegin \keywords{generative AI \and orchestration \and knowledge work transformation \and LLM-based research automation \and distributed cognition \and sociotechnical systems}
\DIFaddend 

% ============================================================
\begin{abstract}
This position paper argues that generative AI is triggering a structural shift in knowledge work: from \textit{creator} to \textit{orchestrator}. To substantiate this argument, we present the \textit{Open Paper Machine}, an autonomous LLM-based research agent built on Claude Opus 4.6 and the Model Context Protocol, which executed the full academic pipeline---literature search, theoretical framing, drafting, figure generation, and \LaTeX{} compilation---producing this manuscript. Drawing on distributed cognition \citep{hutchins1995cognition}, sociotechnical systems theory \citep{trist1951social}, and the task-based automation framework \citep{autor2015why}, we interpret what this demonstration means. The central argument: the researcher's role \DIFdelbegin \DIFdel{shifts }\DIFdelend \DIFaddbegin \DIFadd{is likely to shift }\DIFaddend from producing text to directing, evaluating, and taking accountability for AI-generated output. If the process of desk research---literature reviews, theoretical syntheses, structured argumentation---becomes \DIFdelbegin \DIFdel{fully }\DIFdelend \DIFaddbegin \DIFadd{largely }\DIFaddend automatable, then process alone is no longer a contribution. This \DIFdelbegin \DIFdel{forces }\DIFdelend \DIFaddbegin \DIFadd{might increasingly bring }\DIFaddend science back to what it always should have been: the generation of genuinely new knowledge.
\DIFdelbegin \DIFdel{Research is where the implications are sharpest, because the stakes---epistemic integrity, accountability, the training of future scholars---are highest.
...
Generative AI is transforming the practice of research. Not in the distant future---now. Autonomous systems already search databases, synthesize literature, draft manuscripts, and generate figures \citep{yamada2025scientist,ifargan2024autonomous,asai2024openscholar,zheng2025automation}. The question is no longer whether AI can participate in the research process, but \DIFdelbegin \DIFdel{what }\DIFdelend \DIFaddbegin \DIFadd{how }\DIFaddend the researcher's role \DIFdelbegin \DIFdel{becomes }\DIFdelend \DIFaddbegin \DIFadd{transforms }\DIFaddend when the mechanical phases of that process---the prose writing, the citation formatting, the literature tabulation---become delegable.

Throughout this paper, ``we'' refers to the human authors (Blask and Funk). We---not the AI---designed the research, \DIFdelbegin \DIFdel{selected the theories}\DIFdelend \DIFaddbegin \DIFadd{evaluated and redirected the theoretical framing}\DIFaddend , evaluated every output, and take \DIFdelbegin \DIFdel{full }\DIFdelend responsibility for the published result.

This paper was produced by the system it describes. Claude Opus 4.6---an autoregressive transformer developed by Anthropic---searched four academic databases, \DIFdelbegin \DIFdel{selected theoretical lenses, }\DIFdelend built a concept matrix, and drafted every section you are reading. The human authors did not write the prose. They directed the process, evaluated the output, and \DIFdelbegin \DIFdel{corrected errors}\DIFdelend \DIFaddbegin \DIFadd{changed the wording of the final document}\DIFaddend . We call the system the \textit{Open Paper Machine}: an open, reproducible pipeline that any researcher can deploy, audit, and extend. The reflexive structure---a system writing about itself---provides a direct demonstration: the paper is simultaneously the claim and the evidence.
...
\begin{description}[style=nextline, leftmargin=1em]
  ...
  \item[\textbf{RQ2.}] How does the creator-to-orchestrator transition restructure the role of the \DIFdelbegin \DIFdel{research scientist}\DIFdelend \DIFaddbegin \DIFadd{researcher}\DIFaddend , and what are the implications for authorship, accountability, and the organization of scientific labor?
\end{description}
...
\begin{enumerate}
  ...
  \item \textbf{Architectural blueprint with reflexive demonstration.} The \textit{Open Paper Machine}---an open, extensible LLM-based research pipeline built on Claude Opus 4.6 and the Model Context Protocol---serves as a reference design and as evidence for the position above. This manuscript, produced by the system it describes, constitutes a documented end-to-end demonstration with all intermediate artifacts archived, \DIFdelbegin \DIFdel{compared against six existing }\DIFdelend \DIFaddbegin \DIFadd{situated within the landscape of }\DIFaddend autonomous research systems \citep{handler2023taxonomy,wang2024survey}, and analyzed from the inside.
\end{enumerate}

% [R2-22: §1.5 Roadmap deleted per Burkhardt R2 — "bringt nix"]

% ============================================================
%DIF >  [R3-R2: §2 split into Technical Foundations + Theoretical Lenses per Burkhardt]
\section{\DIFdelbegin \DIFdel{Theoretical }\DIFdelend \DIFaddbegin \DIFadd{Technical }\DIFaddend Foundations}
\DIFdelbegin %DIFDELCMD < \label{sec:theory}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{sec:tech}
\DIFaddend 

%DIF <  [R2-31: Hallucination caveat moved to opening of Section 2]
\DIFdelbegin %DIFDELCMD < \noindent%%%
\textit{\DIFdel{A preliminary note on hallucination.}} %DIFAUXCMD
\DIFdel{Any assessment of }\DIFdelend \DIFaddbegin \DIFadd{This section addresses the technical preconditions for }\DIFaddend LLM-based research \DIFdelbegin \DIFdel{must confront the hallucination problem: LLMs generate plausible but factually incorrect content, including fabricated citations \mbox{%DIFAUXCMD
\citep{alkaissi2023hallucinations,zhang2025hallucination}}\hskip0pt%DIFAUXCMD
. \mbox{%DIFAUXCMD
\citet{asai2024openscholar} }\hskip0pt%DIFAUXCMD
show that without retrieval augmentation, GPT-4o fabricates citations 78--90\% of the time on scientific synthesis tasks. Avoidance is especially important in the scientific context, where a fabricated citation is an integrity violation, not merely a quality problem. The Open Paper Machine addresses this through retrieval-first citation: every reference originates from live API responses with real DOIs, not from the model's parametric memory (see Section~\ref{sec:capabilities}). Synthesis, argumentation, and interpretation remain products of generation and therefore carry inherent hallucination risk}\DIFdelend \DIFaddbegin \DIFadd{pipelines: transformer capabilities, alignment training, tool-augmented agency, and hallucination mitigation}\DIFaddend .
...
%DIF >  [R3-R2: Hallucination section merged — preliminary note + §2.7 combined per Burkhardt #7]
\DIFaddbegin \subsection{\DIFadd{Hallucination and Its Mitigation}}

\DIFadd{Any assessment of LLM-based research must confront the hallucination problem: LLMs generate plausible but factually incorrect content, including fabricated citations \mbox{%DIFAUXCMD
\citep{alkaissi2023hallucinations,zhang2025hallucination}}\hskip0pt%DIFAUXCMD
. \mbox{%DIFAUXCMD
\citet{asai2024openscholar} }\hskip0pt%DIFAUXCMD
show that without retrieval augmentation, GPT-4o fabricates citations 78--90\% of the time on scientific synthesis tasks. The problem demands architectural, not merely behavioral, mitigation. \mbox{%DIFAUXCMD
\citet{zhang2025hallucination} }\hskip0pt%DIFAUXCMD
classify hallucinations as input-conflicting, context-conflicting, and fact-conflicting; for research agents, fact-conflicting hallucinations are most dangerous because they are internally coherent and stylistically appropriate.
}

\DIFadd{The Open Paper Machine implements retrieval-first citation: every reference originates from live API responses (Semantic Scholar, OpenAlex, CrossRef, arXiv) with verifiable DOIs, not from the model's parametric memory, following the retrieval-augmented generation (RAG) architecture validated by \mbox{%DIFAUXCMD
\citeauthor{asai2024openscholar}}\hskip0pt%DIFAUXCMD
's~(\mbox{%DIFAUXCMD
\citeyear{asai2024openscholar}}\hskip0pt%DIFAUXCMD
) OpenScholar system (see Section~\ref{sec:capabilities}). \mbox{%DIFAUXCMD
\citet{guan2024knowledge} }\hskip0pt%DIFAUXCMD
propose knowledge graph grounding as a complementary strategy. Despite these mitigations, synthesis and argumentation remain products of generation and carry residual hallucination risk that only human verification can address.
}

%DIF >  [R3-R2: Related Systems reframed per Burkhardt #10 — not a comparison, these are systems informing this work]
\subsection{\DIFadd{Related Systems}}

\DIFadd{Table~\ref{tab:systems} presents autonomous AI research systems that informed the design of the Open Paper Machine. These are the systems that collectively define the landscape from which our pipeline emerged---not competitors, but reference points that clarify shared capabilities (tool-augmented generation, iterative refinement) and design choices specific to our approach (reflexive self-analysis, IS-grounded theory, extensible MCP architecture).
}

\begin{table}[ht]
\centering
\small
\caption{\DIFaddFL{Autonomous AI research systems.}}
\label{tab:systems}
\begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}X l c c c c >{\raggedright\arraybackslash}p{2.2cm} @{}}
\toprule
\textbf{\DIFaddFL{System}} & \textbf{\DIFaddFL{Scope}} & \textbf{\DIFaddFL{Exec.}} & \textbf{\DIFaddFL{Self-A.}} & \textbf{\DIFaddFL{Review}} & \textbf{\DIFaddFL{Emp.Val.}} & \textbf{\DIFaddFL{Tool Arch.}} \\
\midrule
\DIFaddFL{Coscientist \mbox{%DIFAUXCMD
\citep{boiko2023autonomous} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{Narrow }& \DIFaddFL{Lab }& \DIFaddFL{No }& \DIFaddFL{No }& \DIFaddFL{Yes }& \DIFaddFL{Lab instr. }\\
\DIFaddFL{ChemCrow \mbox{%DIFAUXCMD
\citep{bran2024chemistry} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{Narrow }& \DIFaddFL{Sim. }& \DIFaddFL{No }& \DIFaddFL{No }& \DIFaddFL{Yes }& \DIFaddFL{Chem. tools }\\
\DIFaddFL{AI Scientist-v2 \mbox{%DIFAUXCMD
\citep{yamada2025scientist} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{Full paper }& \DIFaddFL{ML }& \DIFaddFL{No }& \DIFaddFL{Yes }& \DIFaddFL{Yes }& \DIFaddFL{Code exec. }\\
\DIFaddFL{data-to-paper \mbox{%DIFAUXCMD
\citep{ifargan2024autonomous} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{Full paper }& \DIFaddFL{Data }& \DIFaddFL{No }& \DIFaddFL{No }& \DIFaddFL{Yes }& \DIFaddFL{Stat. tools }\\
\DIFaddFL{CycleResearcher \mbox{%DIFAUXCMD
\citep{weng2024cycleresearcher} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{Paper+rev. }& \DIFaddFL{NLP }& \DIFaddFL{No }& \DIFaddFL{Yes }& \DIFaddFL{Yes }& \DIFaddFL{Code exec. }\\
\DIFaddFL{SciAgents \mbox{%DIFAUXCMD
\citep{ghafarollahi2024sciagents} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{Hypothesis }& \DIFaddFL{No }& \DIFaddFL{No }& \DIFaddFL{No }& \DIFaddFL{Part. }& \DIFaddFL{Multi-agent }\\
\DIFaddFL{OpenScholar \mbox{%DIFAUXCMD
\citep{asai2024openscholar} }\hskip0pt%DIFAUXCMD
}& \DIFaddFL{Synthesis }& \DIFaddFL{No }& \DIFaddFL{No }& \DIFaddFL{No }& \DIFaddFL{Yes }& \DIFaddFL{RAG (45M papers) }\\
\textbf{\DIFaddFL{Open Paper Machine}} & \textbf{\DIFaddFL{Full paper}} & \textbf{\DIFaddFL{No}} & \textbf{\DIFaddFL{Yes}} & \textbf{\DIFaddFL{Part.}} & \textbf{\DIFaddFL{No}} & \textbf{\DIFaddFL{MCP (ext.)}} \\
\bottomrule
\end{tabularx}

\vspace{0.5em}
\footnotesize\noindent\textit{\DIFaddFL{Exec.}}\DIFaddFL{~= experimental execution; }\textit{\DIFaddFL{Self-A.}}\DIFaddFL{~= reflexive self-analysis (for OPM: this paper's approach of analyzing its own production, not a built-in system feature); }\textit{\DIFaddFL{Review}}\DIFaddFL{~= automated review loop; }\textit{\DIFaddFL{Emp.Val.}}\DIFaddFL{~= empirical validation of generated claims.
}\end{table}

\DIFadd{The Open Paper Machine differs from these systems not in capability but in intent: it treats its own production process as primary evidence for a theoretical argument about the restructuring of knowledge work. Its limitation is clear: no primary data generation and no automated peer review loop. Its claims rest on the published descriptions of the systems listed; independent benchmarking would strengthen them.
}

%DIF >  ============================================================
%DIF >  [R3-R2: New §3 — Theoretical lenses split from former §2 per Burkhardt #4/#5]
\section{\DIFadd{Theoretical Lenses}}
\label{sec:lenses}

\DIFadd{The creator-to-orchestrator argument draws on three theoretical frameworks: \mbox{%DIFAUXCMD
\citeauthor{autor2015why}}\hskip0pt%DIFAUXCMD
's~(\mbox{%DIFAUXCMD
\citeyear{autor2015why}}\hskip0pt%DIFAUXCMD
) task-based automation theory (automation restructures tasks within occupations, not occupations themselves), \mbox{%DIFAUXCMD
\citeauthor{hutchins1995cognition}}\hskip0pt%DIFAUXCMD
's~(\mbox{%DIFAUXCMD
\citeyear{hutchins1995cognition}}\hskip0pt%DIFAUXCMD
) distributed cognition (research cognition is distributed across human, model, tools, and infrastructure), and \mbox{%DIFAUXCMD
\citeauthor{trist1951social} }\hskip0pt%DIFAUXCMD
and Bamforth's~(\mbox{%DIFAUXCMD
\citeyear{trist1951social}}\hskip0pt%DIFAUXCMD
) sociotechnical systems perspective (technology deployment without institutional redesign produces dysfunction).
}

\DIFaddend \subsection{Distributed Cognition}
...
%DIF <  [R2-28/29: Gap detection and task formulation foregrounded; subsection retitled]
...
%DIF <  [Figure removed — task redistribution now visualized in the central Creator-to-Orchestrator framework (Fig 2)]
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  [R2-31: §2.7 shortened — key content moved to Section 2 opening caveat]
\subsection{\DIFdel{Hallucination Mitigation}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{The hallucination problem (introduced at the opening of this section) demands architectural, not merely behavioral, mitigation. \mbox{%DIFAUXCMD
\citet{zhang2025hallucination} }\hskip0pt%DIFAUXCMD
classify hallucinations as input-conflicting, context-conflicting, and fact-conflicting; for research agents, fact-conflicting hallucinations are most dangerous because they are internally coherent and stylistically appropriate. The Open Paper Machine implements retrieval-first citation: every reference originates from live API responses (Semantic Scholar, OpenAlex, CrossRef, arXiv) with verifiable DOIs, following the RAG architecture validated by \mbox{%DIFAUXCMD
\citeauthor{asai2024openscholar}}\hskip0pt%DIFAUXCMD
's~(\mbox{%DIFAUXCMD
\citeyear{asai2024openscholar}}\hskip0pt%DIFAUXCMD
) OpenScholar system. \mbox{%DIFAUXCMD
\citet{guan2024knowledge} }\hskip0pt%DIFAUXCMD
propose knowledge graph grounding as a complementary strategy. Despite these mitigations, synthesis and argumentation remain products of generation and carry residual hallucination risk that only human verification can address.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Related Systems}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Table~\ref{tab:comparison} positions the Open Paper Machine relative to seven existing autonomous AI research systems. The comparison clarifies what is shared (tool-augmented generation, iterative refinement) and what distinguishes our approach (reflexive self-analysis, IS-grounded theory, extensible MCP architecture).
}%DIFDELCMD < 
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Comparison of autonomous AI research systems.}}
%DIFAUXCMD
%DIFDELCMD < \label{tab:comparison}
%DIFDELCMD < \begin{tabularx}{\textwidth}{@{} >{\raggedright\arraybackslash}X l c c c c >{\raggedright\arraybackslash}p{2.2cm} @{}}
%DIFDELCMD < \toprule
%DIFDELCMD < %%%
\textbf{\DIFdelFL{System}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Scope}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Exec.}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Self-A.}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Review}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Emp.Val.}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Tool Arch.}} %DIFAUXCMD
%DIFDELCMD < \\
%DIFDELCMD < \midrule
%DIFDELCMD < %%%
\DIFdelFL{Coscientist \mbox{%DIFAUXCMD
\citep{boiko2023autonomous} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{Narrow }%DIFDELCMD < & %%%
\DIFdelFL{Lab }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{Yes }%DIFDELCMD < & %%%
\DIFdelFL{Lab instr. }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{ChemCrow \mbox{%DIFAUXCMD
\citep{bran2024chemistry} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{Narrow }%DIFDELCMD < & %%%
\DIFdelFL{Sim. }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{Yes }%DIFDELCMD < & %%%
\DIFdelFL{Chem. tools }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{AI Scientist-v2 \mbox{%DIFAUXCMD
\citep{yamada2025scientist} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{Full paper }%DIFDELCMD < & %%%
\DIFdelFL{ML }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{Yes }%DIFDELCMD < & %%%
\DIFdelFL{Yes }%DIFDELCMD < & %%%
\DIFdelFL{Code exec. }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{data-to-paper \mbox{%DIFAUXCMD
\citep{ifargan2024autonomous} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{Full paper }%DIFDELCMD < & %%%
\DIFdelFL{Data }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{Yes }%DIFDELCMD < & %%%
\DIFdelFL{Stat. tools }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{CycleResearcher \mbox{%DIFAUXCMD
\citep{weng2024cycleresearcher} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{Paper+rev. }%DIFDELCMD < & %%%
\DIFdelFL{NLP }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{Yes }%DIFDELCMD < & %%%
\DIFdelFL{Yes }%DIFDELCMD < & %%%
\DIFdelFL{Code exec. }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{SciAgents \mbox{%DIFAUXCMD
\citep{ghafarollahi2024sciagents} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{Hypothesis }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{Part. }%DIFDELCMD < & %%%
\DIFdelFL{Multi-agent }%DIFDELCMD < \\
%DIFDELCMD < %%%
\DIFdelFL{OpenScholar \mbox{%DIFAUXCMD
\citep{asai2024openscholar} }\hskip0pt%DIFAUXCMD
}%DIFDELCMD < & %%%
\DIFdelFL{Synthesis }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{No }%DIFDELCMD < & %%%
\DIFdelFL{Yes }%DIFDELCMD < & %%%
\DIFdelFL{RAG (45M papers) }%DIFDELCMD < \\
%DIFDELCMD < %%%
\textbf{\DIFdelFL{Open Paper Machine}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Full paper}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{No}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Yes}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{Part.}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{No}} %DIFAUXCMD
%DIFDELCMD < & %%%
\textbf{\DIFdelFL{MCP (ext.)}} %DIFAUXCMD
%DIFDELCMD < \\
...
\textit{\DIFdelFL{Exec.}}%DIFAUXCMD
\DIFdelFL{~= experimental execution; }\textit{\DIFdelFL{Self-A.}}%DIFAUXCMD
\DIFdelFL{~= reflexive self-analysis; }\textit{\DIFdelFL{Review}}%DIFAUXCMD
\DIFdelFL{~= automated review loop; }\textit{\DIFdelFL{Emp.Val.}}%DIFAUXCMD
\DIFdelFL{~= empirical validation of generated claims.
}%DIFDELCMD < \end{table}
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF <  [R2-32: Comparison claim tempered]
\DIFdel{Within this landscape, the Open Paper Machine occupies a distinctive position---not because of superior capability, but because of its design intent. It is, to our knowledge, the only system that treats its own production process as primary evidence for a theoretical argument about the restructuring of knowledge work. Its limitation is clear: no primary data generation and no automated peer review loop. Its comparison-based claims rest on the published descriptions of the systems listed; independent benchmarking would strengthen them.
}%DIFDELCMD < 
...
%DIF >  [R3: Bold removed per Burkhardt — "no bold face"]
This is a position paper that uses \DIFdelbegin \textbf{\DIFdel{design science research}} %DIFAUXCMD
\DIFdelend \DIFaddbegin \DIFadd{design science research }\DIFaddend (DSR) \citep{hevner2004design} as its methodological vehicle. The primary contribution is the \textit{argument}---the creator-to-orchestrator thesis---not the artifact alone. The artifact is the \textit{Open Paper Machine}: the pipeline that produces this manuscript, including its orchestration logic, tool composition, and quality-gate structure. The manuscript is the artifact's \textit{output} and simultaneously the evidence for the position we advance. This dual role---artifact producing evidence about itself---is the reflexive twist\DIFdelbegin \DIFdel{that distinguishes our approach}\DIFdelend .
...
\DIFdelbegin \DIFdel{Following DSR conventions, we evaluate the artifact against two objectives: }\textit{\DIFdel{academic quality}} %DIFAUXCMD
\DIFdel{(rigorous argument, appropriate citation, transparent limitations) and }\textit{\DIFdel{evidential value}} %DIFAUXCMD
\DIFdel{(what the production process reveals about agent capabilities and the orchestrator role).
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend %DIF >  [R3: #32/#33 — Reduced to one objective per Burkhardt; "All three are inseparable" deleted]
The DSR artifact \DIFdelbegin \DIFdel{is threefold: (1)~}\DIFdelend \DIFaddbegin \DIFadd{comprises }\DIFaddend the \textit{Open Paper Machine pipeline}\DIFdelbegin \DIFdel{---its architecture, tool stack, and orchestration logic; (2)~}\DIFdelend \DIFaddbegin \DIFadd{, }\DIFaddend this \textit{manuscript} as \DIFdelbegin \DIFdel{the pipeline's output; and (3)~}\DIFdelend \DIFaddbegin \DIFadd{its output, and }\DIFaddend the \textit{production process} \DIFdelbegin \DIFdel{itself, }\DIFdelend documented through archived artifacts.
\DIFdelbegin \DIFdel{All three are inseparable: the pipeline is evaluated through the paper it produces, and the paper is evidence for the pipeline's capabilities.
}\DIFdelend 

\DIFdelbegin \DIFdel{The evaluation strategy follows }\DIFdelend \DIFaddbegin \DIFadd{Following }\DIFaddend \citeauthor{hevner2004design}'s~(\citeyear{hevner2004design}) guidelines\DIFdelbegin \DIFdel{and combines three elements}\DIFdelend \DIFaddbegin \DIFadd{, we evaluate in three steps}\DIFaddend . First, \textit{technical evaluation}: \DIFdelbegin \DIFdel{the system's output is compared against six existing autonomous research systems (Table~\ref{tab:comparison}), demonstrating both capabilities and deficiencies. }\DIFdelend Section~\ref{sec:capabilities} documents what the pipeline achieved and where it failed\DIFaddbegin \DIFadd{, situated within the landscape of autonomous research systems (Table~\ref{tab:systems})}\DIFaddend . Second, \textit{reflexive evaluation}: \DIFdelbegin \DIFdel{the production process itself is }\DIFdelend \DIFaddbegin \DIFadd{our orchestrator interventions are }\DIFaddend analyzed as data \DIFdelbegin \DIFdel{, with orchestrator interventions documented in the project repository }\DIFdelend (see ``Orchestrator trace'' below and Appendix~\ref{app:timeline}). Third, \textit{theoretical \DIFdelbegin \DIFdel{evaluation}\DIFdelend \DIFaddbegin \DIFadd{interpretation}\DIFaddend }: \DIFdelbegin \DIFdel{the system is interpreted through established theoretical lenses---distributed }\DIFdelend \DIFaddbegin \DIFadd{Section~\ref{sec:orchestrator} frames the production experience through distributed }\DIFaddend cognition, sociotechnical systems, and task-based \DIFdelbegin \DIFdel{automation---to assess whether it produces insights that extend existing understanding (Section~\ref{sec:orchestrator})}\DIFdelend \DIFaddbegin \DIFadd{automation theory}\DIFaddend .

%DIF >  [R3: Time tracking limitation added per Burkhardt — "we should have monitored engagement"]
We acknowledge that \DIFdelbegin \DIFdel{the DSR }\DIFdelend \DIFaddbegin \DIFadd{our }\DIFaddend evaluation has inherent limitations. \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{We did not systematically track the time invested in orchestration (prompt formulation, output evaluation, verification, revision), which would have strengthened claims about the human effort required. Future replications should log orchestrator engagement time alongside pipeline artifacts. Additionally, the }\DIFaddend artifact has not undergone independent external evaluation (e.g., blind expert assessment of manuscript quality or controlled comparison with human-authored papers). The reflexive design---the system evaluating its own output---creates a circularity that artifact archiving mitigates but cannot eliminate. As a position paper, we do not claim that these evaluations validate the creator-to-orchestrator thesis; rather, they provide grounded evidence---from a single, documented case---for the position we advance. We treat this as a first-cycle DSR contribution \citep{hevner2004design}, inviting the research community to conduct the independent evaluation that a self-describing system cannot provide for itself.

\DIFdelbegin \DIFdel{We adopt critical realism \mbox{%DIFAUXCMD
\citep{bhaskar1978realist} }\hskip0pt%DIFAUXCMD
as the ontological position: the mechanisms that produce agentic research behavior exist independently of our descriptions, but our descriptions are theory-laden and shaped by the conditions of their production. The reflexive dimension---the system analyzing itself---follows \mbox{%DIFAUXCMD
\citet{walsham2006doing} }\hskip0pt%DIFAUXCMD
and creates distinctive epistemic risks that we address through artifact archiving and orchestrator documentation. \mbox{%DIFAUXCMD
\citet{rahwan2019machine} }\hskip0pt%DIFAUXCMD
advocate studying machine behavior empirically; this paper constitutes such an observation, with the DSR artifact as both the object and instrument of study.
}\DIFdelend %DIF >  [R3: Critical realism paragraph deleted per Burkhardt — "delete"]
...
% [R2-41/42: Human quality gates made explicit in pipeline description and figure caption]
    \caption{Six-phase Open Paper Machine pipeline: Reconnaissance $\rightarrow$ Framing $\rightarrow$ \DIFdelbeginFL \DIFdelFL{Architecture }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Structure }\DIFaddendFL $\rightarrow$ Production $\rightarrow$ Assembly $\rightarrow$ Self-Review. Each phase transition includes a \textbf{human quality gate}: the orchestrator reviews the phase output, approves or rejects it, and may redirect the agent before the next phase begins. No phase proceeds without human approval. \DIFaddbeginFL \DIFaddFL{Note: the diagram presents an idealized sequence; in practice, phases involve iteration (e.g., returning from Phase~2 to expand the Phase~1 corpus) and the orchestrator performs additional editing on the final manuscript beyond the depicted gates.}\DIFaddendFL }
    \label{fig:pipeline_phases}
\end{figure}

The six-phase pipeline is depicted in Figure~\ref{fig:pipeline_phases}. Each phase produces persistent Markdown artifacts that serve as both intermediate outputs and inputs to subsequent phases. Critically, each phase boundary \DIFdelbegin \DIFdel{constitutes }\DIFdelend \DIFaddbegin \DIFadd{enables }\DIFaddend a \textbf{human quality gate}: the orchestrator reviews the artifact, decides whether it meets the required standard, and either approves progression or redirects the agent. Table~\ref{tab:quality_gates} specifies the gate criteria for each phase.
...
3. \DIFdelbeginFL \DIFdelFL{Architecture }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{Structure }\DIFaddendFL & \texttt{paper\_\DIFdelbeginFL \DIFdelFL{architecture}\DIFdelendFL \DIFaddbeginFL \DIFaddFL{structure}\DIFaddendFL .md} & Argument structure sound? Sections balanced? \\
...
Phase~6 (Self-Review) closes the feedback loop: the system generates a simulated peer review that the orchestrator uses as a checklist. This paper's self-review identified the circularity problem (now addressed in the ``Orchestrator trace'') and asymmetries in the comparative analysis (corrected in Table~\DIFdelbegin \DIFdel{\ref{tab:comparison}}\DIFdelend \DIFaddbegin \DIFadd{\ref{tab:systems}}\DIFaddend ). The self-review does not replace human peer review---it cannot assess novelty or field fit---but it operationalizes the orchestrator principle: the system generates critique, the human evaluates which critiques are valid.
...
\begin{itemize}[nosep]
  ...
  %DIF >  [R3: Verification process clarified per Burkhardt — "manual process?"]
  \item \textit{Citation verification (Phase 4):} Of 62 cited references, 4 contained hallucinated claims (correct paper, incorrect attributed finding). The \DIFdelbegin \DIFdel{orchestrator identified these through spot-checking against }\DIFdelend \DIFaddbegin \DIFadd{pipeline retrieves papers automatically via API; the orchestrator then manually spot-checked attributed claims against the }\DIFaddend retrieved abstracts and corrected \DIFdelbegin \DIFdel{them}\DIFdelend \DIFaddbegin \DIFadd{misattributions}\DIFaddend .
...
%DIF <  [R2-45: Labeled as evaluation criteria; R2-46: "static" removed; R2-47: Reflexivity reframed as caveat]
\DIFdelbegin \subsection{\DIFdel{Evaluation Criteria}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{We evaluate the artifact and its output against four criteria following \mbox{%DIFAUXCMD
\citet{lincoln1985naturalistic}}\hskip0pt%DIFAUXCMD
, supplemented by a methodological caveat specific to reflexive AI research:
}%DIFDELCMD < 

%DIFDELCMD < \begin{itemize}
%DIFDELCMD <   \item %%%
\textbf{\DIFdel{Credibility:}} %DIFAUXCMD
\DIFdel{Transparent process, traceable artifacts, established theoretical grounding. Assessed through artifact inspection and theoretical coherence.
  }%DIFDELCMD < \item %%%
\DIFdelend %DIF >  [R3: #54 — §3.3 Evaluation Criteria (Lincoln & Guba) deleted per Burkhardt — "advertisement, later on we don't do that"]
%DIF >  Reflexivity caveat retained as standalone paragraph:
\textbf{\DIFdelbegin \DIFdel{Transferability:}%DIFDELCMD < \MBLOCKRIGHTBRACE %%%
\DIFdel{Open-source codebase enables replication assessment. The production system is released as a reusable Claude Code plugin (}%DIFDELCMD < \url{https://github.com/TobiasBlask/open-paper-machine}%%%
\DIFdel{).
  }%DIFDELCMD < \item %%%
\textbf{\DIFdel{Dependability:}} %DIFAUXCMD
\DIFdel{The pipeline follows a fixed six-phase structure with a defined tool set, though outputs are stochastic. Dependability is established through artifact archiving: any researcher can inspect the same inputs and trace how outputs were produced.
  }%DIFDELCMD < \item %%%
\textbf{\DIFdel{Confirmability:}} %DIFAUXCMD
\DIFdel{All intermediate artifacts archived for independent audit.
}%DIFDELCMD < \end{itemize}
%DIFDELCMD < 

%DIFDELCMD < %%%
\textbf{\DIFdel{Methodological caveat (reflexivity):}} %DIFAUXCMD
\DIFdel{The system }\DIFdelend \DIFaddbegin \DIFadd{Methodological caveat (reflexivity).}} \DIFadd{The system }\DIFaddend cannot introspect its own weights or training data. Its self-assessment is shaped by the same training that produced the capabilities being assessed. \DIFdelbegin \DIFdel{This is not an evaluation criterion but a structural limitation: self-reported }\DIFdelend \DIFaddbegin \DIFadd{Self-reported }\DIFaddend limitations are a lower bound, and findings about the system's capabilities must be interpreted with this circularity in mind.
...
\textbf{Access bias.} No paywalled content. Systematic bias toward open-access, English-language, computationally-oriented literature. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Epistemic Boundaries}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Four epistemic boundaries and one methodological limitation:
}%DIFDELCMD < 

%DIFDELCMD < \begin{enumerate}
%DIFDELCMD <   \item %%%
\textbf{\DIFdel{Training cutoff.}} %DIFAUXCMD
\DIFdel{Parametric knowledge }\DIFdelend \DIFaddbegin \DIFadd{Parametric knowledge is }\DIFaddend frozen at training time\DIFdelbegin \DIFdel{. Post-cutoff developments }\DIFdelend \DIFaddbegin \DIFadd{; post-cutoff developments are }\DIFaddend only partially compensated by retrieval.
\DIFdelbegin %DIFDELCMD < \item %%%
\DIFdelend \DIFaddbegin 

%DIF >  [R3: §5.3 Epistemic Boundaries deleted per Burkhardt — "we touched the points, delete here"]
%DIF >  Key remaining points folded into §5.2 above and the paragraph below.

\DIFaddend \textbf{\DIFdelbegin \DIFdel{Hallucination}\DIFdelend \DIFaddbegin \DIFadd{No primary data}\DIFaddend .} \DIFdelbegin \DIFdel{Fabricated citations, plausible but inaccurate claims \mbox{%DIFAUXCMD
\citep{alkaissi2023hallucinations,zhang2025hallucination,li2023halueval}}\hskip0pt%DIFAUXCMD
. All claims require human verification.
  }%DIFDELCMD < \item %%%
\textbf{\DIFdel{Overconfident synthesis.}} %DIFAUXCMD
\DIFdel{Sweeping claims that elide contradictions, null results, and methodological limitations in underlying work.
  }%DIFDELCMD < \item %%%
\textbf{\DIFdel{No primary data (current).}} %DIFAUXCMD
\DIFdel{Cannot }\DIFdelend \DIFaddbegin \DIFadd{The system cannot }\DIFaddend conduct interviews, experiments, or \DIFdelbegin \DIFdel{surveys---the system }\DIFdelend \DIFaddbegin \DIFadd{surveys---it }\DIFaddend is fundamentally archival and synthetic \citep{zheng2025automation}. \DIFdelbegin \DIFdel{This is a limitation of the current architecture, not a permanent boundary: }\DIFdelend API-accessible survey platforms \DIFdelbegin \DIFdel{(e.g., Prolific, MTurk) }\DIFdelend could in principle be integrated as MCP tools, \DIFdelbegin \DIFdel{enabling agent-administered questionnaires. Primary data collection is therefore an engineering problem, not an impossibility---but one }\DIFdelend \DIFaddbegin \DIFadd{but }\DIFaddend with significant ethical implications (informed consent, IRB oversight) that would require dedicated governance design.
%DIF <  [R2-50: Evaluation bias moved to methodological limitation context]
...
%DIF <  [Figure removed — epistemic boundaries already enumerated in text above; visual added no analytical value]
\DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend % [R2-57: DSR evaluation summary — concrete assessment against criteria]
\subsection{Evaluation Summary}

%DIF >  [R3: §5.4 tightened — advertisement removed, evaluation claims softened, duplicate orchestrator trace mention fixed per Burkhardt]
Returning to the DSR \DIFdelbegin \DIFdel{evaluation }\DIFdelend criteria defined in Section~\ref{sec:method}: \textit{Technical\DIFdelbegin \DIFdel{evaluation}\DIFdelend }---the pipeline executed all six phases and produced a complete, compilable manuscript with 62 verified references. \DIFdelbegin \DIFdel{Table~\ref{tab:comparison} positions the result relative to seven comparable systems: the Open Paper Machine is the only system combining full-pipeline scope, reflexive self-analysis, and extensible MCP architecture, but it lacks primary data generation and automated peer review. }\DIFdelend The failure modes documented above \DIFdelbegin \DIFdel{(hallucination, quality assessment, access bias, intellectual risk) }\DIFdelend constitute honest capability boundaries\DIFdelbegin \DIFdel{, not disqualifying defects. }\DIFdelend \DIFaddbegin \DIFadd{. }\DIFaddend \textit{Reflexive\DIFdelbegin \DIFdel{evaluation}\DIFdelend }---the orchestrator trace (Section~\ref{sec:method}\DIFdelbegin \DIFdel{) documents five categories of substantive human intervention, demonstrating that orchestration is intellectual work , not }\DIFdelend \DIFaddbegin \DIFadd{; Appendix~\ref{app:timeline}) documents substantive human interventions, providing evidence that orchestration involves intellectual work beyond }\DIFaddend mere approval. \DIFdelbegin \DIFdel{The orchestrator trace (Appendix~\ref{app:timeline}) documents all substantive interventions. }\DIFdelend \textit{Theoretical \DIFdelbegin \DIFdel{evaluation}\DIFdelend \DIFaddbegin \DIFadd{interpretation}\DIFaddend }---Section~\ref{sec:orchestrator} \DIFdelbegin \DIFdel{applies three theoretical lenses to }\DIFdelend \DIFaddbegin \DIFadd{frames }\DIFaddend the production experience \DIFaddbegin \DIFadd{through three theoretical lenses}\DIFaddend , generating the creator-to-orchestrator framework\DIFdelbegin \DIFdel{as the primary analytical contribution}\DIFdelend . Whether this framework constitutes a genuine \DIFdelbegin \DIFdel{theoretical }\DIFdelend advance is a judgment we invite the research community to make; what we can demonstrate is that \DIFdelbegin \DIFdel{the framework }\DIFdelend \DIFaddbegin \DIFadd{it }\DIFaddend emerges from, and is grounded in, the documented production process.
...
\begin{figure}[htbp]
    ...
    \caption{The creator-to-orchestrator shift in knowledge work. \textit{Left:} the traditional creator model where the researcher executes all tasks. \textit{Center:} \citeauthor{autor2015why}'s~(\citeyear{autor2015why}) task redistribution---routine cognitive tasks are delegated to AI, judgment-intensive tasks are retained by the human, and new orchestration tasks emerge from the transition. \textit{Bottom:} the orchestrator model where the researcher directs, evaluates, and \DIFdelbeginFL \DIFdelFL{is accountable }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{takes accountability }\DIFaddendFL while the AI agent executes under human oversight via quality gates. The central implication: if desk research becomes automatable, process execution alone can no longer constitute an academic contribution.}
...
%DIF >  [R3: A5 — Rebalanced "bring science back" claim per Burkhardt — the point is judgment, not polishing]
The positive reading---and the one we endorse---is that automation \textit{\DIFdelbegin \DIFdel{purifies}\DIFdelend \DIFaddbegin \DIFadd{clarifies}\DIFaddend } rather than devalues \DIFdelbegin \DIFdel{science}\DIFdelend \DIFaddbegin \DIFadd{the nature of scientific contribution}\DIFaddend . Much of academic work today is bound up in process: formatting citations, tabulating literature, producing competent prose. These activities consume enormous time and effort but do not, in themselves, generate new knowledge. If the process becomes delegable, science can refocus on what \DIFdelbegin \DIFdel{it always should have been: the generation of genuinely novel insights---new }\DIFdelend \DIFaddbegin \DIFadd{genuinely constitutes a contribution: novel }\DIFaddend research questions, original data, theoretical innovations, methodological advances. \DIFaddbegin \DIFadd{We acknowledge the tension: the orchestrator's daily practice involves evaluating, verifying, and correcting AI output---activities that may resemble quality assurance more than original scholarship. But the judgment exercised in this process---deciding which research questions merit pursuit, which theoretical framings are productive, which claims survive scrutiny---is itself the intellectual contribution that distinguishes orchestrated research from fully autonomous generation. }\DIFaddend Whether those insights come from humans, from human-AI collaboration, or eventually from autonomous systems is a secondary question. The primary question is whether they are \textit{genuine}.
...
%DIF >  [R3: D9 — Strengthened: human value = judgment + accountability, not just oversight]
  \item \textbf{Contribution disclosure} replaces binary authorship. Rather than asking ``who wrote this?'', the relevant question becomes ``who is accountable for these claims\DIFdelbegin \DIFdel{?''---a question that has a clear answer }\DIFdelend \DIFaddbegin \DIFadd{, and what substantive judgment shaped them?''---questions that have clear answers }\DIFaddend under the orchestrator model. \DIFaddbegin \DIFadd{Without such judgment, the appropriate response is not to list a human as author but to let the AI system operate fully autonomously---and to evaluate the output on its own terms.
}\DIFaddend \end{enumerate}

\DIFdelbegin \DIFdel{This framework aligns with }\DIFdelend %DIF >  [R3: "emphasize here" — Burkhardt, p.13: strengthen envelopment point]
\DIFaddbegin \DIFadd{This framework operationalizes }\DIFaddend \citeauthor{asatiani2021envelopment}'s~(\citeyear{asatiani2021envelopment}) sociotechnical envelopment: the AI's output is wrapped in organizational processes---verification, disclosure, audit trails---that provide the accountability the technology itself cannot. \DIFaddbegin \DIFadd{Envelopment is not a metaphor here; it is the mechanism that makes AI-assisted authorship defensible. Without it, AI-generated research is unaccountable output. With it, the orchestrator's judgment transforms that output into a scholarly contribution for which a human bears responsibility.
}\DIFaddend 
...
\subsection{The \DIFdelbegin \DIFdel{Research Scientist}\DIFdelend \DIFaddbegin \DIFadd{Researcher}\DIFaddend 's Evolving Role}

The creator-to-orchestrator transition does not eliminate the \DIFdelbegin \DIFdel{research scientist}\DIFdelend \DIFaddbegin \DIFadd{researcher}\DIFaddend . It restructures the role.
\DIFdelbegin \DIFdel{Three dimensions of restructuring deserve attention.
}\DIFdelend 

%DIF >  [R3: #68 — "Epistemic" deleted from subheading per Burkhardt]
\textbf{\DIFdelbegin \DIFdel{Epistemic }\DIFdelend \DIFaddbegin \DIFadd{Evaluative }\DIFaddend judgment becomes more important, not less.} When AI generates fluent, convention-following prose, the scarce resource is no longer the ability to write---it is the ability to evaluate what is written. Distinguishing genuinely insightful synthesis from fluent recombination requires deep domain knowledge, theoretical sophistication, and what we might call \textit{epistemic \DIFdelbegin \DIFdel{taste}\DIFdelend \DIFaddbegin \DIFadd{discernment}\DIFaddend }\DIFdelbegin \DIFdel{---an aesthetic sense for }\DIFdelend \DIFaddbegin \DIFadd{---the trained ability to judge }\DIFaddend which arguments are productive and which are merely competent. This capacity develops through years of scholarly immersion, not through tool operation.
...
%DIF >  [R3: #69 — "Institutional" changed per Burkhardt — "this term needs to change"]
\textbf{\DIFdelbegin \DIFdel{Institutional }\DIFdelend \DIFaddbegin \DIFadd{Contextual }\DIFaddend knowledge becomes essential.} Research is not merely cognitive work. It is institutionally embedded: conferences have norms, journals have preferences, reviewers have expectations, funding bodies have priorities. This \DIFdelbegin \DIFdel{institutional }\DIFdelend \DIFaddbegin \DIFadd{contextual }\DIFaddend knowledge---knowing which arguments will land at ICIS versus MISQ, how to frame a contribution for a European versus American audience, when to be provocative and when to be conservative---is not learnable from training data. It requires the kind of socialization that doctoral programs, research groups, and conference communities provide.
...
\citet{degallier2022augmentation} }\hskip0pt%DIFAUXCMD
...
}\DIFdelend %DIF >  [R3: Dégallier-Rochat paragraph deleted per Burkhardt — "we said that" — point already made in §6.1]
...
%DIF <  ============================================================
%DIF <  [R2-48: Ethical Considerations relocated here from §3.4]
\DIFdelbegin \section{\DIFdel{Ethical Considerations}}
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
%DIFDELCMD < \label{sec:ethics}
%DIFDELCMD < %%%
\DIFdelend %DIF >  [R3: #70 — §7 Ethical Considerations deleted per Burkhardt — "delete, we said that"
%DIF >    Authorship accountability covered in §6.4, integrity in §5/§6.5, bias in §5.2]

\DIFdelbegin \DIFdel{The reflexive design---an AI system analyzing its own production---raises distinctive ethical questions.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\textbf{\DIFdel{Authorship.}} %DIFAUXCMD
\DIFdel{The authors (Blask and Funk) take full responsibility for the published content. Claude Opus 4.6 is disclosed as the text generator. The system cannot accept moral or legal accountability, and current norms \mbox{%DIFAUXCMD
\citep{flanagin2023nonhuman} }\hskip0pt%DIFAUXCMD
preclude AI authorship. This is not merely a pragmatic accommodation---it instantiates the orchestrator model: the human retains accountability precisely because the human retains evaluative authority.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\textbf{\DIFdel{Integrity.}} %DIFAUXCMD
\DIFdel{AI-generated papers raise questions about fabrication, hallucinated citations, and contribution to the scholarly record \mbox{%DIFAUXCMD
\citep{cotton2023cheating,kendall2024risks}}\hskip0pt%DIFAUXCMD
. We address these through full disclosure, citation verification, and artifact archiving.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\textbf{\DIFdel{Bias.}} %DIFAUXCMD
\DIFdel{The system inherits biases from training data and RLHF evaluators. Literature selection over-represents English-language, Western, computationally-oriented sources. This is a structural limitation, not a correctable error.
}%DIFDELCMD < 
...
\DIFdelbegin \DIFdel{Together, CRediT disclosure and the orchestration trace operationalize the accountability-based authorship model proposed in Section~\ref{sec:orchestrator}: they make the human--AI division of labor transparent, auditable, and comparable across papers.
}\DIFdelend %DIF >  [R3: B11 — Summary sentence deleted per Burkhardt — "delete"]

\subsection{The Hard Problem}

%DIF >  [R3: #72 — "this paper demonstrates..." clause deleted per Burkhardt]
The technology is ready. The institutions are not. The hard problem is not whether AI can produce research-quality \DIFdelbegin \DIFdel{manuscripts---this paper demonstrates that it can, within documented constraints}\DIFdelend \DIFaddbegin \DIFadd{manuscripts}\DIFaddend .
...
\citet{trist1951social} showed seventy-five years ago that mechanization without organizational redesign produces dysfunction. \citet{asatiani2021envelopment} translate this for AI: inscrutable systems require sociotechnical envelopment. The research community has a window---narrow, closing---to design \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend enveloping structures proactively rather than retrofitting them after dysfunction has already propagated.
